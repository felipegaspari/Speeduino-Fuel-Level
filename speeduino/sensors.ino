...
uint16_t readFuelLevel()
{
  uint16_t tempReading;
  byte counterInterval;

  if (configPage13.fuelLevelEnable > 0)
  {
    
    fuelLevelCounter += 1;

    tempReading = analogRead(pinFuelLevel);
    tempReading = analogRead(pinFuelLevel);

    if (fuelLevelHasRun < 1 ) {
      ...
      if ( configPage13.fuelLevelFull > configPage13.fuelLevelEmpty  ){
        ...
        mathMax = configPage13.fuelLevelFull * 20.46;
      }
      tempFuelLevelFloat = constrain(tempReading, mathMin, mathMax);
      tempFuelLevel = tempFuelLevelFloat;
      tempFuelLevelAveraged = tempFuelLevelFloat;
      fuelLevelHasRun = true;
    }
  
    if (tempReading > tempFuelLevelFloat) {
      tempFuelLevelFloat = ((tempFuelLevelFloat * 59) + tempReading) / 60;
    }
    else {
      if (tempReading == tempFuelLevelFloat) {
        tempFuelLevelFloat = tempReading;
      }
      else {
        tempFuelLevelFloat = ((tempFuelLevelFloat * 49) + tempReading) / 50;
      }
    }
    
    switch (configPage13.fuelLevelAvgSpeed){
      case 0: counterInterval = 2; break;
      case 1: counterInterval = 4; break;
      case 2: counterInterval = 8; break;
      case 3: counterInterval = 12; break;
    }
...
//    Serial.print(tempFuelLevelAveraged); 
//          Serial.print(",");
//                      Serial.print("mathMin:");
//    Serial.print(mathMin); 
//          Serial.print(",");
...
//          Serial.print(",");
      ...
      if ( tempFuelLevel >= (configPage13.fuelLevelHalf * 20.26) ){tempFuelLevelMapped = map(tempFuelLevel, (configPage13.fuelLevelHalf * 20.26), mathMax , (mathCapacity / 2), mathCapacity);}
      if ( tempFuelLevel < (configPage13.fuelLevelHalf * 20.26)  ){tempFuelLevelMapped = map(tempFuelLevel, mathMin, (configPage13.fuelLevelHalf * 20.26) , 0, (mathCapacity / 2));}
...
//              Serial.print("mathCapacity");
//    Serial.print(mathCapacity); 
//          Serial.print(",");
////            Serial.print("mathMax");
////    Serial.print(mathMax); 
////          Serial.print(",");
//          Serial.print("tempFuelLevelFloat");
//    Serial.print(tempFuelLevelFloat); 
//          Serial.print(",");
//        Serial.print("tempFuelLevelAveraged");
//    Serial.print(tempFuelLevelAveraged); 
//          Serial.print(",");
//      Serial.print("tempFuelLevel");
//    Serial.print(tempFuelLevel); 
//          Serial.print(",");
      
//      if ( configPage13.fuelLevelFull < configPage13.fuelLevelEmpty  ){tempFuelLevelMapped = constrain(tempFuelLevelMapped, 0, mathCapacity);}
